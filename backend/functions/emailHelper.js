...
export const sendPasswordResetOTPEmail = async(receiverEmail, receiverName, otp) =>{
    ...
    const response = await axios.post(
      ...
      {
        sender: { email: process.env.SENDER_EMAIL_ID , name: 'Multi-service App' },
        to: [{ email: receiverEmail, name: receiverName }],
        subject: 'Multi-service Password Reset verification OTP',
...
export const sendNewPasswordEmail = async(receiverEmail, receiverName) =>{
    ...
    const response = await axios.post(
      process.env.BREVO_EMAIL_API_ENDPOINT,
      {
        sender: { email: process.env.SENDER_EMAIL_ID , name: 'Multi-service App' },
        to: [{ email: receiverEmail, name: receiverName }],
        subject: "Welcome to Multi-service App",
        htmlContent: `Welcome to Multi-service App. You have successfully registered your account with email: `+receiverEmail+"\n\nThank you."
      },
      {
        headers: {
          'accept': 'application/json',
